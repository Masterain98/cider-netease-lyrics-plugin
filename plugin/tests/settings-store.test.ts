...
describe("plugin settings persistence", () => {
  it("defaults new and legacy configurations to direct while preserving a gateway URL", () => {
    ...
    expect(DEFAULT_SETTINGS.convertLyricsToTraditional).toBe(false);
    expect(STORED_DEFAULT_SETTINGS.convertLyricsToTraditional).toBeNull();
    ...
    expect(migrateSettings({ convertLyricsToTraditional: true }, false).convertLyricsToTraditional).toBe(true);
    expect(migrateSettings({ convertLyricsToTraditional: "invalid" as never }, true).convertLyricsToTraditional).toBe(true);
  ...
  it("loads the host config and debounces local changes back to PluginKit", async () => {
    ...
    bindSettings(config, true);

    expect(settings.autoMatch).toBe(false);
    expect(settings.gatewayUrl).toBe("https://gateway.example");
    expect(settings.convertLyricsToTraditional).toBe(true);
    expect(config.value.convertLyricsToTraditional).toBe(true);
